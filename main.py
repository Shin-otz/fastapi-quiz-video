...
def make_video_from_layers(layers_data: List[dict], output_path: str):
    ...
    for l in layers_data:
        total_duration = max(total_duration, float(l.get("endTime", 0)))

    canvas_width = 1280
    canvas_height = 720
    # 전체 배경
    base_clip = ColorClip((canvas_width, canvas_height), color=(255, 255, 255)).set_duration(total_duration)
    ...
    for layer in layers_data:
        ...
        if t == "image":
            ...
            if img_url and Path(img_url).exists():
                iclip = (ImageClip(img_url)
                         .resize(width=w)
                         .set_opacity(opacity)
                         .set_start(start)
                         .set_duration(dur)
                         .set_position((x, y)))
                video_clips.append(iclip)
        ...
        # 텍스트
            ...
            # lineHeight / verticalAlign 처리 (MoviePy 기본은 제한적)
            txt_clip = (TextClip(txt, fontsize=font_size, color=color, method='caption', size=(w, h), align=align)
                        .set_start(start)
                        .set_duration(dur)
                        .set_position((x, y)))
        ...
        # 카운트다운
            ...
            for i in range(count, 0, -1):
                cc = (TextClip(str(i), fontsize=font_size, color=color)
                      .set_start(start + (count - i))
                      .set_duration(1)
                      .set_position((x, y)))
                video_clips.append(cc)

        # 오디오
        elif t == "audio":
            audio_url = layer.get("audioUrl")
            if audio_url and Path(audio_url).exists():
                aclip = AudioFileClip(audio_url).set_start(start)
                audio_clips.append(aclip)

    final_audio = CompositeAudioClip(audio_clips) if audio_clips else None
    final_video = CompositeVideoClip([base_clip] + video_clips).set_audio(final_audio)
